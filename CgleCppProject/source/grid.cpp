...
Grid::Grid(Parameter::Constraint& constraint) {
  m_details = make_unique<GridDetails>();
  m_grid = Eigen::MatrixXcf<complex<double>>(m_details->GetNumXPts(), m_details->GetNumYPts());
...
Grid::Grid(int num_x_pts, int num_y_pts, int num_z_pts, int num_pts,
...
void Grid::PerturbGrid(const double pertubationCoefficient) {
  for (int xPoint = 0; xPoint < this->m_details.m_x_pts.size(); xPoint++) {
    for (int timePoint = 0; timePoint < this->m_details.m_time_pts.size(); timePoint++) {
...
void Grid::PerturbGridHelper(const double& position, const double& time, const Cell& cell,
  ...
  complex<double> = this->m_functHdl.B()(position, time);
