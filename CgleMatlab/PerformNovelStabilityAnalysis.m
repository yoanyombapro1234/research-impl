function grid = PerformNovelStabilityAnalysis(functions, grid, plotResults)
    ...
    grid.PerturbedGridA(isnan(grid.PerturbedGridA))=0;
    grid.PerturbedGridB(isnan(grid.PerturbedGridB))=0;
    
    grid.PerturbedGridAA = grid.PerturbedGridA;
    grid.PerturbedGridBB = grid.PerturbedGridB;
    
    grid.PerturbedGridBB(:, 1) = 0;
    grid.PerturbedGridBB(:, end) = 0;
    
    grid.PerturbedGridAA(:, 1) = 0;
    grid.PerturbedGridAA(:, end) = 0;
    ...
    if metadata.Version == 2
        ...
        for i = 2:length(grid.T)-1 % T > delta T ... we skip the 1st time point since it is the initial condition and last time point since it is the boundary condition
         ...
         if plotResults == true
               ...
               axis on;
               %hold on;
               %shold1 = mesh(grid.X,grid.T,abs(grid.GroundTruthB));
